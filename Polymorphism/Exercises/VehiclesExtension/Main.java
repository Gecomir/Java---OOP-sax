...
public class Main {
    ...
    private static void getVehicle(Scanner console) {
        ...
        switch (attributes[0]) {
            case "Car":
                car = new Car(Double.parseDouble(attributes[1]), Double.parseDouble(attributes[2]),
                        Double.parseDouble(attributes[3]));
                break;
            case "Truck":
                truck = new Truck(Double.parseDouble(attributes[1]), Double.parseDouble(attributes[2]),
                        Double.parseDouble(attributes[3]));
                break;
            case "Bus":
                bus = new Bus(Double.parseDouble(attributes[1]), Double.parseDouble(attributes[2]),
                        Double.parseDouble(attributes[3]));
                break;
        }
    }
